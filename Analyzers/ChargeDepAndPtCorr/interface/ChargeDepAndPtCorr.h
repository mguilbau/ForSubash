...
class ChargeDepAndPtCorr : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
   public:
      ...
      double getEff(TLorentzVector pvector, int evtclass);
   ...
   private:
      ...
      // bool cweight2_; //Apply (True) or not (False) the eff/fake correction
      edm::InputTag fname_; //Name of the file that contains the eff corrections
      edm::InputTag fname2_; //Name of the file that contains the eff corrections
      ...
      // TFile* feff2_; //ROOT File that contains histograms used for corrections
      ...
      double d0d0error_pix_; //maximum DCA - z significance to be a pixel track
