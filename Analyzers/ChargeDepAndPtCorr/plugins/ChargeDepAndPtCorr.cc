...
ChargeDepAndPtCorr::ChargeDepAndPtCorr(const edm::ParameterSet& iConfig) :
  ...
  d0d0error_pix_(iConfig.getUntrackedParameter<double>("d0d0error_pix")),  
  ...
  nPhiBins_(iConfig.getUntrackedParameter<int>("nPhiBins"))
{
   ...
   hCounts_          = fGlobalHist.make<TH1I>("hCounts", "",  2, 0, 2);

   hNtrigs_          = fGlobalHist.make<TH1I>("hNtrigs", "",  2, 0, 2);
   ...
   //****************QA plots for the data set*******************                                       
   hPhidist_ = fGlobalHist.make<TH1F>("hPhidist", "", 31, -TMath::Pi(),  TMath::Pi() ); //31
   hEtadist_ = fGlobalHist.make<TH1D>("hEtadist", "",32, -2.4, 2.4); //32, -2.4, 2.4
   hPTdist_ = fGlobalHist.make<TH1D>("hPTdist", "", 30, 0.0, 3.0); //30
   //************************************************************
...
ChargeDepAndPtCorr::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   switch(evtclassifier_)
     {
       ...
       //evtclass = centBin/2;
       evtclass = centBin;
       if(evtclass < centmin_*2 || evtclass >= centmax_*2) return;
       break;
     case 1:
       evtclass = noff;
   ...
   // ----- Calotower selection -----
   if(isHI_) LoopCaloTower(iEvent, iSetup);
                             ...
                             const edm::EventSetup& iSetup)
{
        ...
        ++itTrk )
   {
       ...
       if( fabs(dzvtx / dzerror)   > 39.0 ) continue;
       if( fabs(dxyvtx / dxyerror) > 9.0 ) continue;
                               ...
                               bool istrg, int evtclass)
{
        ...
        ++itTrk )
   {
       ...
       if(isHI_ && isPix_ )
	 {
	   ...
	   if( pt <= pTmax_pix_         &&
	       ...
	       fabs(dxyvtx / dxyerror) <  d0d0error_pix_ )
	     goodpixtrk = true;
   ...
   if(istrg)
   {
      ...
      hNtrigs_->AddBinContent(1,nTrkTot_corr_trg_);
      for(unsigned int itrg = 0; itrg < pTmin_trg_.size(); ++itrg)
      {
   ...
   else
   {
      ...
      hNtrigs_->AddBinContent(2,nTrkTot_corr_ass_);
...
ChargeDepAndPtCorr::GetEffWeight(double eta, double pt, int evtclass)
{
  ...
  effweight = heff_[centIdx]->GetBinContent(
					    heff_[centIdx]->GetXaxis()->FindBin(eta), 
					    heff_[centIdx]->GetYaxis()->FindBin(pt)
					    );
...
ChargeDepAndPtCorr::GetEffWeight(double eta, double pt, int evtclass)
{
  ...
  if(evtclass == -1)
  ...
  else
    {
      ...
      for(int icent = 0; icent < static_cast<int>(effCorrBinMin_.size()); ++icent)
	{
	  if(evtclass >= effCorrBinMin_[icent]*2 && evtclass < effCorrBinMax_[icent]*2)
	    {
	      centIdx = icent;
	      continue;
	    }
	}
      effweight = heff_[centIdx]->GetBinContent(heff_[centIdx]->GetXaxis()->FindBin(eta), heff_[centIdx]->GetYaxis()->FindBin(pt));
    }
  return effweight;
}
...
ChargeDepAndPtCorr::FillHistsSignal(int ievt)
{
  for( unsigned int itrg = 0; itrg < pTmin_trg_.size(); itrg++ )
  {
    for( unsigned int jass = 0; jass < pTmin_ass_.size(); jass++ )
    {
      ...
      //double trigEff=1.0, assEff=1.0; //new by Prabhat Oct 19
	...
	for( unsigned int nass = 0; nass < nasssize; nass++ )
	    ...
	    {
		//double effweight = trigEff * assEff;
		
		hSignal_[itrg][jass]->Fill( fabs(deltaEta),deltaPhi,1.0/4.0/effweight );
		hSignal_[itrg][jass]->Fill(-fabs(deltaEta),deltaPhi,1.0/4.0/effweight );
		hSignal_[itrg][jass]->Fill( fabs(deltaEta),deltaPhi2,1.0/4.0/effweight);
		hSignal_[itrg][jass]->Fill(-fabs(deltaEta),deltaPhi2,1.0/4.0/effweight );
  ...
  for( unsigned int itrg = 0; itrg < pTmin_trg_.size(); itrg++ )
  {
	...
	//if( itrg < jass ) continue;
	  ...
	  //double trigEff=1.0, assEff=1.0; //new by Prabhat Oct 19
	    ...
	    //if (cweight_) trigEff=getEff(pvector_trg,ievt_trg);
		...
		//if (cweight_) assEff=getEff(pvector_ass,jevt_ass);
...
//-------------------------------------------
double
ChargeDepAndPtCorr::getEff(const TLorentzVector pvector, int evtclass)
{
   double effweight = 1.0;
   for(int i_cf=0; i_cf < static_cast<int>(effCorrBinMin_.size()); i_cf++) if(evtclass >= effCorrBinMin_[i_cf] && evtclass < effCorrBinMax_[i_cf]) {
       effweight = heff_[i_cf]->GetBinContent(
					      heff_[i_cf]->GetXaxis()->FindBin(pvector.Eta()),
					      heff_[i_cf]->GetYaxis()->FindBin(pvector.Pt())
					      );
     }
   return effweight;
   }
